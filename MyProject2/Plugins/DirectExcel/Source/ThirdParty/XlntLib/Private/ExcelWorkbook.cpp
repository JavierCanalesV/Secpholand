...
bool UExcelWorkbook::Load(FString fullPath)
{
	if (fullPath.IsEmpty())
	{
	...
	std::string path = TCHAR_TO_UTF8(*mPath);
...
bool UExcelWorkbook::SaveAs(FString path, bool isAbsolutePath, bool isSave /*= false*/)
{
	...
	if (!isAbsolutePath)
	{
		path = UDirectExcelLibrary::ToAbsolutePath(path, isSave);
	}
...
const TArray<UExcelWorksheet*>& UExcelWorkbook::AllSheets()
{
	return mSheets;
}
