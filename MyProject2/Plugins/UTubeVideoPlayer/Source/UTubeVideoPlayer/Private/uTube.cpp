...
void AuTube::LoadYoutubeUrl(FString _youtubeUrl)
{
	useQuality = false;
	videoEncryptedUrl = "";
...
void AuTube::LoadYoutubeUrlMultiquality(FString _youtubeUrl, TEnumAsByte<Quality> _quality)
{
...
/*Http call*/
void AuTube::MyHttpCall(FString _youtubeUrl)
{
	TSharedRef<IHttpRequest, ESPMode::ThreadSafe> Request = Http->CreateRequest();
	Request->OnProcessRequestComplete().BindUObject(this, &AuTube::OnResponseReceived);
...
bool AuTube::ResponseIsValid(FHttpResponsePtr Response, bool bWasSuccessful) {
...
bool AuTube::IsVideoUnavailable(FString pageSource)
{
	FString unavailableContainer = "<div id=\"watch-player-unavailable\">";
	return pageSource.Contains(unavailableContainer);
}

void AuTube::OnResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful)
{
...
TArray<TSharedPtr<FJsonValue>> AuTube::GetStreamMap(TSharedPtr<FJsonObject> json)
{
...
TArray<TSharedPtr<FJsonValue>> AuTube::GetAdaptiveStreamMap(TSharedPtr<FJsonObject> json)
{
	TSharedPtr<FJsonObject> streamingData = json->GetObjectField("streamingData");
...
TMap<FString, FString> AuTube::ParseQueryString(FString s)
{
	if (s.Contains("?"))
	{
		int32 start = s.Find("?");
...
FString AuTube::GetHtml5PlayerVersionForFix(FString _url)
{
	FString js = _url;
...
FString AuTube::GetHtml5PlayerVersion(TSharedPtr<FJsonObject> jsonObject)
{
	...
	if (myMatcher.FindNext())
	{
		UE_LOG(LogTemp, Warning, TEXT("RETURNED JS"));
		return myMatcher.GetCaptureGroup(1);
	}
	else 
	{
		...
		if (matchNewResult.FindNext()) 
		{
			UE_LOG(LogTemp, Warning, TEXT("RETURNED JS on second level"));
			return matchNewResult.GetCaptureGroup(1);
		}
	}
...
void AuTube::DecryptVideoUrl(FString actualUrl, FString videoUrl, FString htmlVersion)
{
	...
	if (queries.Contains("sig"))
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("EIASUEISAUEI: %s"), *videoEncryptedUrl);
...
void AuTube::GetJsForExtraction(FString url)
{
	TSharedRef<IHttpRequest, ESPMode::ThreadSafe> Request = Http->CreateRequest();
	Request->OnProcessRequestComplete().BindUObject(this, &AuTube::OnResponseReceivedForJS);
	//This is the url on which to process the request
	Request->SetURL(url);
	Request->SetVerb("GET");
...
void AuTube::OnResponseReceivedForJS(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful)
{
	if (ResponseIsValid(Response, bWasSuccessful))
	{
		FString js = Response->GetContentAsString();
		(new FAutoDeleteAsyncTask<VideoDecryptionAsync>(js, sig, lsig, encryptedUrl, videoSig, videoLSig, videoEncryptedUrl, this))->StartBackgroundTask();
	}
}

FString AuTube::GetFunctionFromLine(FString currentLine)
{
...
TArray<TCHAR> AuTube::ReverseFunction(TArray<TCHAR> a)
{
	Algo::Reverse(a);
	return a;
}

TArray<TCHAR> AuTube::SpliceFunction(TArray<TCHAR> a, int32 index)
{
...
TArray<TCHAR> AuTube::SwapFunction(TArray<TCHAR> a, int32 b)
{
	if (a[0] == '\0')
	{
		a.RemoveAt(0);
